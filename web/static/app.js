...
const markerPlugin=LightweightCharts.createSeriesMarkers(series,[]);let lines=[],chosen='',first=true;
...
function probability(id,value){document.querySelector(id).textContent=value==null?'-':(value*100).toFixed(1)+'%'}
function paint(d){
 document.querySelector('#status').textContent='MT5 connesso - '+roma(Math.floor(new Date(d.tick.timestamp).getTime()/1000))+' Roma';
 ...
 const sig=document.querySelector('#signal');sig.textContent=inf.candidate||'-';sig.className=inf.candidate==='BUY'?'buy':inf.candidate==='SELL'?'sell':'';
 series.setData(d.candles);volume.setData(d.candles.map(x=>({time:x.time,value:x.volume,color:x.close>=x.open?'#1f8a78aa':'#b85766aa'})));markerPlugin.setMarkers(d.markers);levels(d.levels);
 ...
 document.querySelector('#cards').innerHTML=d.cards.map(c=>{let p=c.total_pnl||0;return `<article class="card"><h3>${c.name}</h3><div class="pnl ${p>=0?'plus':'minus'}">${p>=0?'+':''}${fmt(p)} USD</div><div class="meta">aperto ${fmt(c.unrealized_pnl)} - chiuso ${fmt(c.realized_pnl)}<br>trade ${c.trades} - posizioni ${c.open_positions} - ${c.last_signal||'N/D'}</div></article>`}).join('');
 if(first){chart.timeScale().fitContent();first=false}
}
async function load(){try{const r=await fetch('/api/snapshot?strategy='+encodeURIComponent(chosen));if(!r.ok)throw new Error((await r.json()).detail);paint(await r.json())}catch(e){document.querySelector('#status').textContent='Errore feed: '+e.message}}
