...
const endpoint=location.pathname==='/indicators'?'/api/indicators':'/api/snapshot';async function load(){try{const r=await fetch(endpoint+'?strategy='+encodeURIComponent(chosen));if(!r.ok)throw new Error((await r.json()).detail);paint(await r.json())}catch(e){document.querySelector('#status').textContent='Errore feed: '+e.message}}
